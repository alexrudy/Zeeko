...
class AddressInfo(object):
    ...
    def __attrs_post_init__(self):
        if self.scheme in ("inproc", "unix"):
            self.url = "{scheme}://{hostname:s}-{port:d}".format(
                port=self.port, scheme=self.scheme, hostname=self.hostname
            )
        else:
            self.url = "{scheme}://{hostname:s}:{port:d}".format(
                port=self.port, scheme=self.scheme, hostname=self.hostname
...
def zmain(docs=None):
    ...
    @click.option(
        "--bind", "bind", help="Try to bind the connection.", flag_value="bind"
    )
    @click.option(
        "--connect",
        "bind",
    ...
    def main(ctx, quiet, port, secondary_port, host, scheme, bind):
        ...
        ctx.obj.secondary = AddressInfo(
            port=secondary_port, scheme=scheme, hostname=host
        )
        if bind != "guess":
